...
| File | Purpose |
| --- | --- |
| `data/sources.csv` | Documents source records, access paths, legal context, join keys, DCAT mapping, and Hugging Face links when relevant. |
| `data/tools.csv` | Documents tools used to discover, extract, clean, validate, publish, or evaluate sources. |
| `data/evaluations.csv` | Stores dated evaluations of source quality, access, legal risk, and recommended actions. |
| `data/use-cases.csv` | Documents practical reuse workflows that connect sources, tools, outputs, audiences, and limits. |
